...
export const Question = ({questionData, isVisible, id, value}) => {
    ...
    if(questionData.type === types.MultipleChoice){
        ...
        question = <RadioBoxField data={questionData.choices} id={id} value={value} />
        ...
        question = <IntegerField id={id}/>
    }else if(questionData.type === types.Decimal){
        question = <FloatField id={id}/>
    ...
    return <div key={id} className="survey-question" style={{display:`${isVisible ? "" : "none"}`}}>
        <div className={`survey-question-label t-label`}>
